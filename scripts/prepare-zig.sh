...
# Keep only what zig cc needs for clang builtins and C target headers.
# We use --sysroot / at runtime so zig uses the Lambda system's CRT and libgcc
# instead of compiling them from source — no glibc/musl/compiler_rt/std needed.
